...
MYBLAS_DEVICE void init_data(half* data, const int M, const int N, const int ld) {
  for (int i = 0; i < M; ++i){
    for (int j = 0; j < N; ++j) {
...
__global__ void test_mma() {
  ...
  HMMA16816(D_reg[0], D_reg[1], A_reg[0], A_reg[1], A_reg[2], A_reg[3], B_reg[0], B_reg[1], C_reg[0], C_reg[1])
...
#pragma unroll
    for (int n=0; n<2; ++n){
      D[smem_out_linear_idx + n] = reinterpret_cast<half *>(&D_reg[reg_linear_idx])[n];
    }
...
TEST(test_mma, HMMA16816) {
  ...
  CHECK_CUDA(cudaMemcpyFromSymbol(&passed_host, passed_device, sizeof(bool), 0, cudaMemcpyDeviceToHost))
