__global__ void sgemmKernel3(const int M, const int N, const int K,
  ...
  for (int dotOrder = 0; dotOrder < K; dotOrder += BK) {
    ...
    sharedA[(sharedACol * 4 + 0) * BM + sharedARow] = tmp.x;
    sharedA[(sharedACol * 4 + 1) * BM + sharedARow] = tmp.y;
    sharedA[(sharedACol * 4 + 2) * BM + sharedARow] = tmp.z;
    sharedA[(sharedACol * 4 + 3) * BM + sharedARow] = tmp.w;
...
#pragma unroll
      for (int i = 0; i < TM; i++)
        vectorOuterA[i] =
            sharedA[innerOuterProdOrder * BM + (threadRow * TM + i)];
#pragma unroll
      for (int i = 0; i < TN; i++)
        vectorOuterB[i] =
