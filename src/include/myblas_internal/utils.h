...
template <typename T>
MYBLAS_HOST_DEVICE bool verify(T const *src, T const *ref, int const length,
                               const double allowedError = 0.0) {
...
template <class T>
void print_matrix(const int nRows, const int nCols, const T *matrixValue,
...
template <>
inline void print_matrix<half>(const int nRows, const int nCols,
                               const half *matrixValue,
                               const ::std::string &label) {
  ::std::unique_ptr<float[]> matrixFloat{
      halfToFloat(matrixValue, nRows * nCols)};
  print_matrix<float>(nRows, nCols, matrixFloat.get(), label);
}

template <typename T>
MYBLAS_HOST_DEVICE void print_value(int const M, int const N,
                                    T const *matrixValue, const char *label) {
  printf("matrix %s\n", label);
  for (int i = 0; i < M; ++i) {
    for (int j = 0; j < N; ++j) {
...
template <typename T>
MYBLAS_HOST_DEVICE void count_result(const T *resultA, const char *nameA,
                                     const T *resultB, const char *nameB,
                                     const int length) {
